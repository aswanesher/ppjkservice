...
class Post_category extends CI_Controller
{
    ...
    redirect('post_category/post_category_add/x1','refresh');
            }
    ...
    redirect('post_category/post_category_edit/'.$id.'/x1','refresh');
            }
        } else {
            redirect('backend_panel', 'refresh');
        }
    ...
    public function post_category_delete($id)
    {
        ...
        if($this->session->userdata('logged_in')) {
            if(!empty($per->is_delete)&&($per->is_delete=='true')) {
                /* PUT YOUR OWN PROCESS HERE */
                if($this->post_category_model->cekdata($id)) {
                redirect('post_category/index/x1','refresh');
                     }
                else {
                         $this->post_category_model->hapus($id);
                    redirect('post_category', 'refresh');
                }
